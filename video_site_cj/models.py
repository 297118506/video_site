...
def init_db():
    ...
    # ===== 图集模块表（需求新增） =====
    _init_album_tables(c)
    _migrate_album_tables(c)
...
def set_user_permissions(user_id, category_ids, category_type='normal'):
    ...
    category_type: 'normal'（长视频）/'short'（短视频）/'album'（图集）。
    """
    conn = get_db()
    c = conn.cursor()
...
def get_user_permissions(user_id):
    ...
    返回: {'normal': [id...], 'short': [id...], 'album': [id...], 'series': bool}
    若 normal/short/album 为空列表，表示该类型全部可见。
    series 为布尔值，True=可访问合集，False=不可访问。
    """
    conn = get_db()
    c = conn.cursor()
    result = {'normal': [], 'short': [], 'album': [], 'series': True}
    for ctype in ('normal', 'short', 'album'):
...
# ============================================================================
# 图集模块（album_categories / albums / album_images）
# ============================================================================

def _init_album_tables(cursor):
    """初始化图集模块三张表。"""
    cursor.execute('''CREATE TABLE IF NOT EXISTS album_categories (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        name TEXT NOT NULL,
        slug TEXT UNIQUE,
        parent_id INTEGER DEFAULT 0,
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS album_images (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
...
def _migrate_album_tables(cursor):
    """为 albums / album_images 表补充缺失字段（向后兼容老库）"""
    # 按表分组：table_name -> [(col_name, col_type)]
    table_cols = {
        'albums': [
            ('image_count', 'INTEGER DEFAULT 0'),
        ],
        'album_images': [
            # 视频支持：'image' | 'video'（老数据默认 image）
...
def get_all_album_categories():
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT * FROM album_categories ORDER BY sort_order, id')
    return [dict(r) for r in c.fetchall()]


def get_album_category(cat_id):
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT * FROM album_categories WHERE id = ?', (cat_id,))
    row = c.fetchone()
    conn.close()
    return dict(row) if row else None


def create_album_category(name, parent_id=0, sort_order=0):
    conn = get_db()
    c = conn.cursor()
    slug = name.lower().replace(' ', '-')
    try:
        c.execute('INSERT INTO album_categories (name, slug, parent_id, sort_order) VALUES (?, ?, ?, ?)',
                  (name, slug, parent_id, sort_order))
        conn.commit()
        return True, c.lastrowid
    except sqlite3.IntegrityError:
...
def update_album_category(cat_id, name=None, parent_id=None, sort_order=None):
    conn = get_db()
    c = conn.cursor()
    fields, vals = [], []
    if name is not None:
        ...
        vals.append(name)
        fields.append('slug = ?')
        vals.append(name.lower().replace(' ', '-'))
    ...
    if not fields:
        ...
        return False
    vals.append(cat_id)
    c.execute(f'UPDATE album_categories SET {", ".join(fields)} WHERE id = ?', vals)
    conn.commit()
    conn.close()
    return True


def _collect_child_album_cat_ids(cursor, parent_ids):
    """递归收集图集分类的所有子孙 ID（含自身）。"""
    all_ids = list(parent_ids)
    current = list(parent_ids)
    while current:
        ph = ','.join(['?' for _ in current])
        cursor.execute(f'SELECT id FROM album_categories WHERE parent_id IN ({ph})', current)
...
def delete_album_category(cat_id):
    conn = get_db()
    c = conn.cursor()
    all_ids = _collect_child_album_cat_ids(c, [cat_id])
    ph = ','.join(['?' for _ in all_ids])
    # 该分类（含子分类）下的图集记录 category_id 重置为 0（不删除图集本身）
    c.execute(f'UPDATE albums SET category_id = 0 WHERE category_id IN ({ph})', all_ids)
    c.execute(f'DELETE FROM album_categories WHERE id IN ({ph})', all_ids)
    conn.commit()
    conn.close()


def batch_delete_album_categories(cat_ids):
    if not cat_ids:
        return 0
    conn = get_db()
    c = conn.cursor()
    all_ids = _collect_child_album_cat_ids(c, list(cat_ids))
    ph = ','.join(['?' for _ in all_ids])
    c.execute(f'UPDATE albums SET category_id = 0 WHERE category_id IN ({ph})', all_ids)
    c.execute(f'DELETE FROM album_categories WHERE id IN ({ph})', all_ids)
    affected = c.rowcount
...
def batch_delete_all_album_categories():
    conn = get_db()
    c = conn.cursor()
    c.execute('UPDATE albums SET category_id = 0 WHERE category_id IN (SELECT id FROM album_categories)')
    c.execute('DELETE FROM album_categories')
    affected = c.rowcount
    conn.commit()
    conn.close()
    return affected
...
def _refresh_album_image_count(conn, album_id):
    """根据 album_images 表的实际行数刷新 albums.image_count。"""
    c = conn.cursor()
    c.execute('SELECT COUNT(*) FROM album_images WHERE album_id = ?', (album_id,))
    count = c.fetchone()[0]
    c.execute('UPDATE albums SET image_count = ? WHERE id = ?', (count, album_id))
    return count


def get_all_albums(category_id=None, keyword=None, page=1, per_page=20, category_ids=None):
    conn = get_db()
    c = conn.cursor()
    q = 'SELECT * FROM albums WHERE 1=1'
    cq = 'SELECT COUNT(*) AS total FROM albums WHERE 1=1'
    params, cparams = [], []
    if category_id:
        q += ' AND category_id = ?'
        cq += ' AND category_id = ?'
        params.append(category_id)
        cparams.append(category_id)
    elif category_ids is not None and len(category_ids) > 0:
        ph = ','.join(['?' for _ in category_ids])
        q += f' AND category_id IN ({ph})'
        cq += f' AND category_id IN ({ph})'
        params.extend(category_ids)
        cparams.extend(category_ids)
    if keyword:
        q += ' AND (title LIKE ? OR description LIKE ?)'
        cq += ' AND (title LIKE ? OR description LIKE ?)'
        params.extend([f'%{keyword}%', f'%{keyword}%'])
        cparams.extend([f'%{keyword}%', f'%{keyword}%'])
    q += ' ORDER BY sort_order, id DESC LIMIT ? OFFSET ?'
    params.extend([per_page, (page - 1) * per_page])

    c.execute(cq, cparams)
    total = c.fetchone()['total']
    c.execute(q, params)
...
def get_album_by_source_path(source, original_path):
    """按 source + 原始路径检查是否已存在图集（去重导入用）。"""
    conn = get_db()
    c = conn.cursor()
    # 用 image_url 与 source 字段做匹配：local 时 original_path 编码进 image_url；这里直接查
    # 由于 image_url 可能是 /static/...（本地复制后），需要根据 source 区分。简化处理：本地按 title 重复不阻断，仅按文件路径查。
    conn.close()
    return None
...
def create_album(data):
    ...
    c.execute('''INSERT INTO albums
                 (title, description, category_id, cover, source, sort_order)
                 VALUES (?, ?, ?, ?, ?, ?)''',
              (data.get('title'), data.get('description', ''),
               data.get('category_id', 0), data.get('cover', ''),
               data.get('source', 'openlist'),
               data.get('sort_order', 0)))
...
def update_album(album_id, data):
    ...
    c = conn.cursor()
    fields, vals = [], []
    for key in ['title', 'description', 'category_id', 'cover', 'source', 'sort_order']:
        if key in data:
            fields.append(f'{key} = ?')
    ...
    c.execute(f'UPDATE albums SET {", ".join(fields)}, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
              vals)
    conn.commit()
    conn.close()


def delete_album(album_id, delete_files=False):
    """删除图集：先删图集图片记录，再删图集。delete_files=True 时同步删除本地复制文件目录。"""
    import shutil
    conn = get_db()
    c = conn.cursor()
    # 取出本地复制路径，以便删除磁盘文件
    if delete_files:
        c.execute('SELECT image_url FROM album_images WHERE album_id = ?', (album_id,))
        local_paths = [r[0] for r in c.fetchall() if r[0] and r[0].startswith('/static/')]
    c.execute('DELETE FROM album_images WHERE album_id = ?', (album_id,))
    c.execute('DELETE FROM albums WHERE id = ?', (album_id,))
    conn.commit()
    conn.close()
    if delete_files and local_paths:
        # 收集父目录（同一 album 的 static 子目录）
        parent_dirs = {os.path.dirname(p.lstrip('/')) for p in local_paths}
        for d in parent_dirs:
            # d like "static/album_images/<album_id>"，拼到用户数据根目录
            # （打包后 BASE_DIR = EXE 同级，保证与导入时写入的位置一致）
            abs_dir = os.path.join(BASE_DIR, d)
            if os.path.isdir(abs_dir):
                shutil.rmtree(abs_dir, ignore_errors=True)


def batch_delete_albums(album_ids, delete_files=False):
    if not album_ids:
        return 0
    import shutil
    conn = get_db()
    ...
    local_parent_dirs = set()
    if delete_files:
        c.execute(f'SELECT image_url FROM album_images WHERE album_id IN ({ph})', album_ids)
        for r in c.fetchall():
            url = r[0] or ''
            if url.startswith('/static/'):
                ...
                if len(parts) >= 3:
                    local_parent_dirs.add('/'.join(parts[:2]))  # static/album_images
    c.execute(f'DELETE FROM album_images WHERE album_id IN ({ph})', album_ids)
    c.execute(f'DELETE FROM albums WHERE id IN ({ph})', album_ids)
    affected = c.rowcount
    conn.commit()
    conn.close()
    if delete_files and local_parent_dirs:
        project_root = BASE_DIR
        for d in local_parent_dirs:
            abs_dir = os.path.join(project_root, d)
            # 仅删除本次涉及到的 album 子目录
            for album_id in album_ids:
                target = os.path.join(abs_dir, str(album_id))
                if os.path.isdir(target):
                    shutil.rmtree(target, ignore_errors=True)
    return affected


def batch_delete_all_albums(delete_files=False):
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT id FROM albums')
    album_ids = [r[0] for r in c.fetchall()]
    conn.close()
    return batch_delete_albums(album_ids, delete_files=delete_files)


def increment_album_views(album_id):
    conn = get_db()
    c = conn.cursor()
    c.execute('UPDATE albums SET views = views + 1 WHERE id = ?', (album_id,))
    conn.commit()
    conn.close()
...
def get_album_images(album_id):
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT * FROM album_images WHERE album_id = ? ORDER BY sort_order, id', (album_id,))
    images = [dict(r) for r in c.fetchall()]
    conn.close()
    return images


def add_album_images(album_id, image_items):
    """批量新增图集媒体（图片或视频），并自动刷新 image_count 与封面。

    image_items: [{image_url, source, original_filename, sort_order, media_type}]
      - media_type: 'image'（默认） | 'video'
      - 视频条目 image_url 存可播放直链（OpenList 走 get_file_link 转直链，本地视频拷贝到 album_dir）
    """
    if not image_items:
        return 0
    conn = get_db()
    ...
    for idx, item in enumerate(image_items):
        # 容错：image / video 之外一律按 image 处理
        mt = (item.get('media_type') or 'image').lower()
        if mt not in ('image', 'video'):
            mt = 'image'
        c.execute('''INSERT INTO album_images
                     (album_id, image_url, media_type, source, original_filename, sort_order)
                     VALUES (?, ?, ?, ?, ?, ?)''',
                  (album_id,
                   item.get('image_url', ''),
                   mt,
                   item.get('source', 'openlist'),
                   item.get('original_filename', ''),
                   item.get('sort_order', idx)))
        inserted += 1
    if inserted:
        _refresh_album_image_count(conn, album_id)
        # 封面永远用首张图片（视频不参与封面，避免 lightbox 封面是视频）
        c.execute("SELECT image_url FROM album_images WHERE album_id = ? AND media_type = 'image' ORDER BY sort_order, id LIMIT 1", (album_id,))
        first_img = c.fetchone()
        if not first_img:
            # 没有任何图片（如纯视频图集），则用首张媒体作为封面兜底
            c.execute('SELECT image_url FROM album_images WHERE album_id = ? ORDER BY sort_order, id LIMIT 1', (album_id,))
            first_img = c.fetchone()
        if first_img:
            c.execute('UPDATE albums SET cover = ? WHERE id = ?', (first_img['image_url'], album_id))
    conn.commit()
    conn.close()
    return inserted


def delete_album_image(image_id, delete_file=False):
    import shutil
    conn = get_db()
    c = conn.cursor()
    c.execute('SELECT album_id, image_url FROM album_images WHERE id = ?', (image_id,))
    row = c.fetchone()
    if not row:
        conn.close()
        return
    album_id, image_url = row['album_id'], row['image_url']
    c.execute('DELETE FROM album_images WHERE id = ?', (image_id,))
    _refresh_album_image_count(conn, album_id)
    # 若该图集无图，更新封面
    c.execute('SELECT image_url FROM album_images WHERE album_id = ? ORDER BY sort_order, id LIMIT 1', (album_id,))
    first = c.fetchone()
    new_cover = first['image_url'] if first else ''
    c.execute('UPDATE albums SET cover = ? WHERE id = ?', (new_cover, album_id))
    conn.commit()
    conn.close()
    if delete_file and image_url.startswith('/static/'):
        project_root = BASE_DIR
        abs_path = os.path.join(project_root, image_url.lstrip('/'))
...
def batch_delete_album_images(image_ids, delete_files=False):
    import shutil, glob
    if not image_ids:
        return 0
    conn = get_db()
    c = conn.cursor()
    ph = ','.join(['?' for _ in image_ids])
    if delete_files:
        c.execute(f'SELECT image_url FROM album_images WHERE id IN ({ph})', image_ids)
        files_to_delete = [r[0] for r in c.fetchall() if r[0] and r[0].startswith('/static/')]
    c.execute(f'SELECT DISTINCT album_id FROM album_images WHERE id IN ({ph})', image_ids)
    affected_album_ids = [r[0] for r in c.fetchall()]
    c.execute(f'DELETE FROM album_images WHERE id IN ({ph})', image_ids)
    affected = c.rowcount
    # 刷新每个受影响图集的封面与数量
    for album_id in affected_album_ids:
        ...
        c.execute('SELECT image_url FROM album_images WHERE album_id = ? ORDER BY sort_order, id LIMIT 1', (album_id,))
        first = c.fetchone()
        c.execute('UPDATE albums SET cover = ? WHERE id = ?',
                  (first['image_url'] if first else '', album_id))
    conn.commit()
    conn.close()
    if delete_files:
        project_root = BASE_DIR
        for f in files_to_delete:
            abs_path = os.path.join(project_root, f.lstrip('/'))
            if os.path.isfile(abs_path):
                try:
                    os.remove(abs_path)
                except OSError:
...
def update_album_images_order(album_id, ordered_image_ids):
    """按传入 ID 顺序重排图集图片。"""
    ...
    for idx, img_id in enumerate(ordered_image_ids):
        c.execute('UPDATE album_images SET sort_order = ? WHERE id = ? AND album_id = ?',
                  (idx, img_id, album_id))
    # 把第一张设为封面
    if ordered_image_ids:
        c.execute('SELECT image_url FROM album_images WHERE id = ?', (ordered_image_ids[0],))
        first = c.fetchone()
        c.execute('UPDATE albums SET cover = ? WHERE id = ?',
                  (first['image_url'] if first else '', album_id))
    conn.commit()
...
def get_album_stats():
    ...
    c = conn.cursor()
    stats = {}
    c.execute('SELECT COUNT(*) AS total, COALESCE(SUM(views), 0) AS total_views FROM albums')
    row = c.fetchone()
    stats['total_albums'] = row['total'] or 0
    stats['total_album_views'] = row['total_views'] or 0
    c.execute('SELECT COUNT(*) FROM album_categories')
    stats['total_album_categories'] = c.fetchone()[0] or 0
    c.execute('SELECT COUNT(*) FROM album_images')
    stats['total_album_images'] = c.fetchone()[0] or 0
    conn.close()
    return stats
