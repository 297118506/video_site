...
def get_shorts_for_random(category_id=None, exclude_played=False, limit=500, user_id=None, allowed_category_ids=None):
    ...
    if user_id is not None:
        ...
        c.execute(f'''SELECT COUNT(*) FROM user_short_played up
                  ...
                  [user_id] + params)
        played_count = c.fetchone()[0]
        played_filter = f''' AND v.id NOT IN (
        ...
        played_count = c.fetchone()[0]
        played_filter = ' AND COALESCE(v.is_played, 0) = 0'
        played_params = list(params)

    used_fallback = False
    if exclude_played:
        query = f'SELECT v.* FROM videos v WHERE v.is_short_video = 1{played_filter}{cat_filter} ORDER BY v.id DESC LIMIT ?'
        c.execute(query, played_params + [limit])
    else:
        query = f'SELECT v.* FROM videos v WHERE v.is_short_video = 1{cat_filter} ORDER BY v.id DESC LIMIT ?'
        c.execute(query, list(params) + [limit])
    videos = [dict(r) for r in c.fetchall()]

    if exclude_played and not videos:
        used_fallback = True
        fb_query = f'SELECT v.* FROM videos v WHERE v.is_short_video = 1{cat_filter} ORDER BY v.id DESC LIMIT ?'
