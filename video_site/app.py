...
from models import (
    ...
    get_all_favorites, delete_favorite, batch_delete_favorites
)
...
def shorts_play(category_id=0):
    ...
    if allowed_ids is not None and category_id == 0 and len(allowed_ids) > 0:
        ...
        conn.close()
        # 播放状态
        played_status = get_short_played_status(None)
    else:
        ...
        videos = result['videos']
        total = result['total']
        played_status = get_short_played_status(cat_kwarg if cat_kwarg != 0 else None)
...
def api_incr_view(video_id):
    ...
    if video.get('is_short_video'):
        marked, reset, played_cnt, total_cnt = mark_short_played(video_id)
        extra['played'] = {
            'marked': marked,
            'reset': reset,
...
def api_v1_short_played_status():
    ...
    """
    category_id = request.args.get('category_id', type=int)
    status = get_short_played_status(category_id)
...
def api_admin_reset_short_played():
    """[后台] 手动清空所有短视频的播放标记。"""
    affected = reset_all_short_played()
...
def api_v1_shorts():
    ...
    # 需求1：exclude_played=1 且无搜索关键词时，走专用查询（优先未播放）
